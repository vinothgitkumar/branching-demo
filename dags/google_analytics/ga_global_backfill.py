...
from plugins.config import GoogleAnalyticsGlobal
...
databricks_cluster_brnz, databricks_cluster_libs = get_bronze_cluster_config(GoogleAnalyticsGlobal.gcp_bq_cred, env)
...
notebook_params = {
    ...
    'env': env,
    'tardis_source': GoogleAnalyticsGlobal.tardis_data_source,
    'dataset_id': GoogleAnalyticsGlobal.project_info_project_id
}

notebook_task_params = {'libraries': databricks_cluster_libs,
                        'notebook_task': {
                            'notebook_path': GoogleAnalyticsGlobal.notebook_backfill,
...
def load_subdag(parent_dag_name, child_dag_name, args, log_date, nb_path, dataset_id, databricks_cluster,
                brand_mapping_notebook=None, load_stats=None, **context):
    dag_subdag = DAG(
    ...
    with dag_subdag:
        ...
        if child_dag_name == 'silver':

            brand_mapping_notebook_databricks_task = DatabricksSubmitRunOperator(
                task_id="brand_mapping_task",
                json={
                    'libraries': databricks_cluster_libs,
                    'notebook_task': {
                        'notebook_path': brand_mapping_notebook,
                        'base_parameters': {
                            'dataset': json.dumps({
                                "log_date": log_date,
                                'tardis_source': GoogleAnalyticsGlobal.tardis_data_source,
            ...
            stats_load_databricks_task = DatabricksSubmitRunOperator(
                task_id="stats_load_task",
                json={
                    'libraries': databricks_cluster_libs,
                    'notebook_task': {
                        'notebook_path': load_stats,
                        'base_parameters': {
                            'dataset': json.dumps({
                                "log_date": log_date,
                                'tardis_source': GoogleAnalyticsGlobal.tardis_data_source,
            ...
            check_task >> create_conn_db >> databricks_task >> brand_mapping_notebook_databricks_task >> stats_load_databricks_task >> end_task
        else:
...
load_bronze_tasks = SubDagOperator(
    ...
    subdag=load_subdag(parent_dag_name=DAG_NAME,
                       ...
                       nb_path=GoogleAnalyticsGlobal.notebook_bronze,
...
load_silver_tasks = SubDagOperator(
    ...
    subdag=load_subdag(parent_dag_name=DAG_NAME,
                       ...
                       args=default_args,
                       log_date="{{ dag_run.conf['logdate'] }}",
                       nb_path=GoogleAnalyticsGlobal.notebook_silver,
                       dataset_id="{{ dag_run.conf['dataset_id'] }}",
                       brand_mapping_notebook=GoogleAnalyticsGlobal.brand_mapping_notebook,
                       load_stats=GoogleAnalyticsGlobal.load_stats,
