...
from plugins.config import SlackConfig, AppConfig
from plugins.utilities.vault import vault_instance

# alert_channels for Success and failure alert

if AppConfig.environment in ['production', 'prod']:
...
def success_alert(context):
    ...
    return send_slack_message(context=context, channel=alert_channel, text=message)
...
def failure_alert(context):
    message = """
        ...
        exec_date=context.get('execution_date'),
        log_url=context.get('task_instance').log_url)
    return send_slack_message(context=context, channel=failure_channel, text=message)
